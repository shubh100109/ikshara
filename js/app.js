...
class LuxuryStoreApp {
        ...
        <div class="product-card group relative bg-white rounded-xl overflow-hidden border border-stone-100/80 hover:border-amber-200/80 transition-all duration-300 flex flex-col justify-between shadow-sm hover:shadow-xl" data-id="${product.id}">
          ...
          <div class="relative aspect-[3/4] w-full overflow-hidden bg-stone-100 cursor-pointer" onclick="app.openQuickView('${product.id}')">
            ...
            <div class="absolute top-2.5 left-2.5 z-10 flex flex-col gap-1 items-start">
              ${product.badge ? `
                <span class="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-stone-950 text-amber-300 rounded shadow">
                  ${product.badge}
                </span>
              ` : ''}
              ${discountPercent > 0 ? `
              ...
              ` : ''}
              ${product.readyToShip ? `
                <span class="px-1.5 py-0.5 text-[9px] font-semibold bg-emerald-700 text-white rounded flex items-center gap-1 shadow">
                  ⚡ Ready to Ship
                </span>
              ` : ''}
            </div>
            ...
            <button 
              ...
              class="absolute top-2.5 right-2.5 z-10 w-8 h-8 rounded-full bg-white flex items-center justify-center text-stone-700 hover:text-red-600 hover:scale-110 shadow transition-all duration-200 border border-stone-200"
            ...
            <div class="absolute inset-x-0 bottom-3 flex justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 px-3 z-10">
              <button 
                ...
                class="w-full py-1.5 px-3 bg-white text-stone-900 text-xs font-semibold rounded-lg shadow-md hover:bg-amber-50 flex items-center justify-center gap-1.5 border border-stone-200">
  ...
  toggleWishlist(productId) {
    if (window.auth && !window.auth.isLoggedIn()) {
      ...
      }, "Please sign in to save this design to your Wishlist ❤️");
      return;
    }
  ...
  initiateWhatsAppBuy(productId, selectedSize = "Standard", customStitching = false) {
    if (window.auth && !window.auth.isLoggedIn()) {
      window.auth.requireAuth(() => {
        this.initiateWhatsAppBuy(productId, selectedSize, customStitching);
      }, "Please sign in to proceed with WhatsApp order 🛍️");
      return;
    }
  ...
  initiateCartWhatsAppBuy() {
    ...
    if (window.auth && !window.auth.isLoggedIn()) {
      window.auth.requireAuth(() => {
        this.initiateCartWhatsAppBuy();
      }, "Please sign in to order your shopping bag via WhatsApp 🛍️");
      return;
    }
  ...
  // ================= HERO SHOWCASE & LANDING PAGE INTERACTION HELPERS =================
  switchHeroShowcase(productId) {
    const product = this.products.find(p => p.id === productId);
    if (!product) return;

    this._activeHeroProductId = productId;
    const imgEl = document.getElementById('heroShowcaseImage');
    const titleEl = document.getElementById('heroShowcaseTitle');
    const fabricEl = document.getElementById('heroShowcaseFabric');
    const priceEl = document.getElementById('heroShowcasePrice');
    const originalPriceEl = document.getElementById('heroShowcaseOriginalPrice');
    const buyBtn = document.getElementById('heroShowcaseBuyBtn');
    const quickBtn = document.getElementById('heroShowcaseQuickBtn');
    ...
    if (fabricEl) fabricEl.textContent = `${product.fabric} • ${product.work}`;
    if (priceEl) priceEl.textContent = this.formatPrice(product.priceINR);
    if (originalPriceEl) originalPriceEl.textContent = this.formatPrice(product.originalPriceINR);
    if (buyBtn) {
      buyBtn.setAttribute('onclick', `app.initiateWhatsAppBuy('${product.id}')`);
    }
    if (quickBtn) {
      quickBtn.setAttribute('onclick', `app.openQuickView('${product.id}')`);
    }

    // Update active tab buttons
    document.querySelectorAll('.hero-look-btn').forEach(btn => {
      if (btn.getAttribute('data-product-id') === productId) {
        ...
        btn.classList.remove('bg-amber-400', 'text-stone-950', 'font-bold');
        btn.classList.add('bg-white/10', 'text-stone-200');
      }
    });
  }

  updateHeroShowcasePrice() {
    const heroId = this._activeHeroProductId || 'sar-kanjivaram-01';
    const product = this.products.find(p => p.id === heroId);
    const priceEl = document.getElementById('heroShowcasePrice');
    const originalPriceEl = document.getElementById('heroShowcaseOriginalPrice');
    if (priceEl && product) {
      priceEl.textContent = this.formatPrice(product.priceINR);
    }
    if (originalPriceEl && product) {
      originalPriceEl.textContent = this.formatPrice(product.originalPriceINR);
    }
  }

  filterBySpotlight(category) {
    this.filters.category = category;
    this.filters.subcategory = 'all';

    // Update left sidebar radio
    ...
    pills.forEach(p => {
      ...
      if (cat === category && !p.getAttribute('data-subcategory')) {
        p.classList.remove('bg-stone-100', 'text-stone-800');
        p.classList.add('active', 'bg-stone-900', 'text-white');
      } else {
        p.classList.remove('active', 'bg-stone-900', 'text-white');
    ...
    this.showToast(`Showing all ${category === 'saree' ? 'Pure Sarees' : 'Three-Piece Suits'}`);
  }

  filterByOccasion(occasion) {
    this.filters.occasion = occasion;
    const occSelect = document.getElementById('occasionFilter');
    if (occSelect) occSelect.value = occasion;

    this.renderProducts();
    document.getElementById('productsSection')?.scrollIntoView({ behavior: 'smooth' });
    this.showToast(`Filtered by ${occasion}`);
  }
